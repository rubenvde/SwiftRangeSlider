...
@IBDesignable open class RangeSlider: UIControl {
    ...
    public typealias RangeChangedAction = (RangeSlider) -> ()
    open var rangeDidChangeAction: RangeChangedAction?
    ...
    @IBInspectable open lazy var minimumAcceptedValue: Double = {
        minimumValue
    }()
    
    ///The maximum accepted value. The slider will not move higher than this value
    @IBInspectable open lazy var maximumAcceptedValue: Double = {
        maximumValue
    }()
    ...
    open var showTextAsPercentage = false
    ...
    open func getLabelText(forValue value: Double) -> String {
        ...
        return labelText + (showTextAsPercentage ? "%" : "")
    }
    ...
    open func updateLabelPositions() {
        ...
        var newLowerLabelCenter = CGPoint(x: lowerKnobCenter.x, y: lowerKnob.frame.origin.y + lowerKnob.frame.height + (lowerLabel.frame.size.height / 2))
        var newUpperLabelCenter = CGPoint(x: upperKnobCenter.x, y: upperKnob.frame.origin.y + upperKnob.frame.height + (upperLabel.frame.size.height / 2))
